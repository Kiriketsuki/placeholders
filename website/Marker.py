...
class Marker():
    ...
    def createMarkers(self, amenityList):
        markerList = [StaticMapMarker(locations=[(i['geometry']['location']['lat'], i['geometry']
                                      ['location']['lng'])], color="red", size="large", label="A") for i in amenityList]

        markerList.append(StaticMapMarker(locations=[
                          (self.fromLat, self.fromLng)], color="purple", size="large", label="T"))
